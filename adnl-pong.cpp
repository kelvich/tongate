...
class Callback : public adnl::Adnl::Callback {
 public:
  void receive_message(AdnlNodeIdShort src, AdnlNodeIdShort dst, td::BufferSlice data) override {
      ...
      std::cout << "got receive_message" << std::endl;
  }
  ...
  void receive_query(AdnlNodeIdShort src, AdnlNodeIdShort dst, td::BufferSlice data,
    ...
    create_serialize_tl_object<ton_api::adnl_ping>();
  }
...
int main(int argc, char *argv[]) {
  ...
  p.add_option('k', "key", "private key", [&](td::Slice key) {
    TRY_RESULT_PREFIX(keydata, td::read_file_secure(key.str()), "failed to read private key: ");
    TRY_RESULT_ASSIGN(pk, ton::PrivateKey::import(keydata));
    return td::Status::OK();
  });
  ...
  std::cout << "pub: " << td::base64_encode(pub.export_as_slice().as_slice()) << std::endl;
  ...
  scheduler.run_in_context([&]() {
    ...
    auto tladdrlist = ton::create_tl_object<ton::ton_api::adnl_addressList>(
        std::move(addr_vec), ton::adnl::Adnl::adnl_start_time(), ton::adnl::Adnl::adnl_start_time(), 0, (int)td::Time::now() + 3600);
